...
namespace LootValue
{
	...
	public class LootValueMod : BaseUnityPlugin
	{
		...
		private void SetupConfig()
		{
			...
			ShowPricePerKgAndPerSlotInRaid = Config.Bind("0. Item Prices", "3. Show price per KG & price per slot in raid", true);
            ...
            // General -> Quick Sell:
            OneButtonQuickSell = Config.Bind("1. Quick Sell", "1. One button quick sell", false,
@"If disabled: 
...
[Alt + Shift + Left Click] sells to either depending on who pays more");
			...
			SellSimilarItems = Config.Bind("1. Quick Sell", "2. Sell all similar items in one go", true, "If you sell one item and have multiple of the same, they will all be sold simultaneously subject to additionally holding down the LControl.");
            EnableQuickSellHint = Config.Bind("1. Quick Sell", "3. Enable tooltip hint", false, "Show or hide the hint in the quick sale tooltip.");
            IgnoreFleaMaxOfferCount = Config.Bind("1. Quick Sell", "4. Ignore flea max offer count", false);
			SellToTraderIfWeaponIsNonOperational = Config.Bind("1. Quick Sell", "5. Sell to trader if item is weapon and non operational", true);
			SellToTraderBelowProfitThresholdEnabled = Config.Bind("1. Quick Sell", "6. Sell to trader if flea market price below threshold enabled", true);
			SellToTraderProfitThreshold = Config.Bind("1. Quick Sell", "6.1. Flea market profit threshold", 5000,
			...
			SellToTraderBelowDurabilityThresholdEnabled = Config.Bind("1. Quick Sell", "7. Sell to trader if below durability threshold enabled", true);
			SellToTraderDurabilityThreshold = Config.Bind("1. Quick Sell", "7.1. Durability threshold %", 60,
